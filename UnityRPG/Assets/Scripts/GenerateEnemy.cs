        ...
        for (int i = 0; i < maxNum; i++)
        {
            ...
            while (!check && count < 3)
            {
                ...
                if (Physics.Raycast(new Vector3(randX, terrain.GetPosition().y + terrain.terrainData.size.y, randZ), Vector3.down, out hit, terrain.GetPosition().y + terrain.terrainData.size.y + 100f, LayerMask.GetMask("Field")))
                {
                    ...
                    if (!Physics.SphereCast(new Vector3(randX, terrain.GetPosition().y + terrain.terrainData.size.y, randZ), radius, Vector3.down, out hit, terrain.GetPosition().y + terrain.terrainData.size.y + 100f, LayerMask.GetMask("Player", "Monster", "Block")))
                    {
